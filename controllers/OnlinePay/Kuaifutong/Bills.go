...
func (k *KuaiPayCore) SignParameters(parameters []map[string]string, encoding string) map[string]string {
	...
	ErrorHelper.LogInfo("待签名数据：", parameters)
	...
	//fmt.Println(parameters)
	//fmt.Println(value)
	ErrorHelper.LogInfo("待签名排序后数据：", rt)
	return rt
}
...
func (k *KuaiPayHelper) Gbp_pay(Money float64) Return_317 {
	...
	_baseParam = append(_baseParam, map[string]string{"orderNo": orderNumber})
	_baseParam = append(_baseParam, map[string]string{"tradeName": "代付"})
	_baseParam = append(_baseParam, map[string]string{"tradeTime": date.FormatDate(time.Now(), "yyMMddHHmmss")})
	_baseParam = append(_baseParam, map[string]string{"currency": "CNY"})
	_baseParam = append(_baseParam, map[string]string{"custProtocolNo": "30000054452726"})       //扣款人在快付通备案的协议号 原来老协议号30000043360199
	_baseParam = append(_baseParam, map[string]string{"custBankAccountNo": "99501880106692899"}) //银行账户 民生银行6232553000519574 郑州银行99501880106692899
	_baseParam = append(_baseParam, map[string]string{"custBankNo": "3134911"})                  ////银行行别 民生银行3051000 郑州银行3134911
	_baseParam = append(_baseParam, map[string]string{"custName": "河南联九舟网络科技有限公司"})
 ...
 */
func (k *KuaiPayHelper) Gbp_send_treaty_record_to_kft() interface{} {
	var _rt interface{}
	...
	_baseParam = append(_baseParam, map[string]string{"merchantId": R2_merchantNo})
	_baseParam = append(_baseParam, map[string]string{"orderNo": _orderNo})
	_baseParam = append(_baseParam, map[string]string{"merchantTreatyNo": _text}) //用户与商户的协议号
	...
	_baseParam = append(_baseParam, map[string]string{"startDate": date.FormatDate(time.Now(), "yyyyMMdd")})
	_baseParam = append(_baseParam, map[string]string{"endDate": "20201231"})
	_baseParam = append(_baseParam, map[string]string{"bankCardType": "1"})
	_baseParam = append(_baseParam, map[string]string{"holderName": "河南联九舟网络科技有限公司"})
	_baseParam = append(_baseParam, map[string]string{"bankType": "3134911"})                 //银行行别 民生银行3051000 郑州银行3134911
	_baseParam = append(_baseParam, map[string]string{"bankCardNo": "99501880106692899"})     //银行账户 民生银行6232553000519574 郑州银行99501880106692899
	_baseParam = append(_baseParam, map[string]string{"certificateType": "0"})                //证件类型不校验
	_baseParam = append(_baseParam, map[string]string{"certificateNo": "41012219710510305X"}) //身份证不校验
	_baseParam = append(_baseParam, map[string]string{"mobileNo": "15638905677"})             //手机号
	...
	fmt.Println(formData)
	_req := k.HttpPostCall(BaseUrl, formData)
	ErrorHelper.LogInfo("318执行结果：", _req)
	err := json.Unmarshal([]byte(_req), &_rt)
	ErrorHelper.CheckErr(err)
	return _rt
}
 ...
 */
func (k *KuaiPayHelper) Gbp_query_treaty_record_info(orderNumber string) interface{} {
	var _rt interface{}
	_baseParam := k.GetBaseParam()
